...
const PRODUSE_FABRICATE = [
  ...
  },
  {
    ...
    categorie: "Flanșe",
    descriere: "Disc plastic cu rozeta extinsă Ø120–140mm, model spite duble. Distribuție uniformă a forței de prindere pe vată minerală și polistiren.",
      ...
      <section className="section-padding bg-white">
        <div className="container-site">
          <div className="flex items-end justify-between mb-8">
            ...
            <span className="text-sm text-slate-500 hidden sm:block">{PRODUSE_FABRICATE.length} produse</span>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6">
            {PRODUSE_FABRICATE.map((p) => (
              <article key={p.slug} className="card flex flex-col group hover:border-brand-blue/30">
                ...
                <div className="mt-auto pt-3 border-t border-neutral-border flex items-center justify-between">
                  <div className="text-xs text-slate-500">
                    <div><span className="font-medium text-slate-700">Variante:</span> {p.variante}</div>
                    <div><span className="font-medium text-slate-700">Livrare:</span> {p.livrare}</div>
                  </div>
