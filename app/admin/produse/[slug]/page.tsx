...
export default function EditProdusFabricatPage() {
  ...
  async function handleSave() {
    ...
    let newData;
    if (isFabricat) {
      newData = { ...allData, fabricate: allData.fabricate.map((p: Produs) => p.slug === slug ? updated : p) };
    } else {
      newData = { ...allData, distribuite: allData.distribuite.map((p: Produs) => p.slug === slug ? updated : p) };
    }
    ...
    try {
      ...
      if (res.ok) { setMessage('Salvat cu succes!'); setAllData(newData); }
  ...
  async function handleDelete() {
    ...
    if (res.ok) router.push('/admin/produse');
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!imagine) setImagine(url);
        else setGalerie((prev) => [...prev, url]);
  ...
  async function handleDeleteImage(imgUrl: string, isMain: boolean) {
    ...
    if (imgUrl.startsWith('/api/img/')) {
      ...
      await fetch('/api/admin/images', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ path }) });
    }
    if (isMain) { setImagine(galerie[0] || ''); setGalerie((prev) => prev.slice(1)); }
    else setGalerie((prev) => prev.filter((u) => u !== imgUrl));
  }
    ...
    <div className="max-w-2xl">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            {imagine && (
              <div className="relative group">
                ...
                <span className="absolute top-1 left-1 text-xs bg-blue-600 text-white px-1.5 py-0.5 rounded font-medium">Main</span>
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          {!isFabricat && (
            <button onClick={handleDelete} className="ml-auto text-sm text-red-500 hover:text-red-700 font-medium">Șterge produs</button>
          )}
        </div>
      </div>
