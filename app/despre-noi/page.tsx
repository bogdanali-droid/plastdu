...
const CLIENTI = [
  ...
  { categorie: 'Distribuitori', descriere: 'Aprovizionăm distribuitori în județele: București, Ilfov, Dolj, Argeș, Vâlcea, Prahova, Dâmbovița, Cluj.',
    icon: (<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 00-10.026 0 1.106 1.106 0 00-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12" /></svg>), },
  { categorie: 'Depozite & Magazine', descriere: 'Aprovizionăm depozite și magazine în: București, Ilfov, Dolj, Gorj, Argeș, Vâlcea, Prahova, Dâmbovița, Brașov, Teleorman.',
...
export default async function DespreNoiPage() {
  ...
  let certificate: Certificat[] = DEFAULT_CERTIFICATE;
  ...
  try {
    ...
    if (kv) {
      ...
      if (storedCert) {
        const parsedCert = JSON.parse(storedCert);
        if (Array.isArray(parsedCert) && parsedCert.length > 0) certificate = parsedCert;
      }
    }
  } catch {}
        ...
        <section className="bg-white border-b border-neutral-border py-10">
          <div className="container-site">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {CIFRE.map((c: { val: string; label: string }) => (
          ...
          <div className="container-site">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <div className="mt-8">
                  ...
                  <ul className="space-y-2">
                    {SPECIALIZARI.map((s: string) => (
                      <li key={s} className="flex items-start gap-3 text-sm text-slate-600">
                        <span className="w-1.5 h-1.5 rounded-full bg-brand-accent flex-shrink-0 mt-2" />{s}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {FABRICA.imagini[0] && (<div className="col-span-2 relative h-56 rounded-2xl overflow-hidden bg-slate-100 img-watermark"><ImageWithFallback src={FABRICA.imagini[0]} alt="Fabrică Plast Du IV — Jilava, Ilfov" className="object-cover" sizes="(max-width: 1024px) 100vw, 50vw" /></div>)}
                {FABRICA.imagini[1] && (<div className="relative h-36 rounded-xl overflow-hidden bg-slate-100 img-watermark"><ImageWithFallback src={FABRICA.imagini[1]} alt="Producție dibluri Plast Du IV" className="object-cover" sizes="25vw" /></div>)}
                {FABRICA.imagini[2] && (<div className="relative h-36 rounded-xl overflow-hidden bg-slate-100 img-watermark"><ImageWithFallback src={FABRICA.imagini[2]} alt="Matriță producție Plast Du IV" className="object-cover" sizes="25vw" /></div>)}
              </div>
            </div>
          </div>
        </section>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {VIDEOCLIPURI.map((url: string, i: number) => (
                  <div key={url} className="rounded-2xl overflow-hidden bg-black shadow-card">
                    <video src={url} controls preload="metadata" className="w-full aspect-video" aria-label={`Videoclip fabricare ${i + 1}`} />
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="section-padding bg-neutral-surface">
          <div className="container-site">
            <div className="grid md:grid-cols-2 gap-8 max-w-4xl">
              ...
              <div className="bg-white rounded-2xl p-6 border border-neutral-border shadow-card">
                <div className="w-10 h-10 rounded-xl bg-brand-accent/10 text-brand-accent flex items-center justify-center mb-4"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.964-7.178z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg></div>
                <h3 className="text-base mb-3">Viziunea noastră</h3>
                <p className="text-sm text-slate-600 leading-relaxed">Să redefinim standardele industriei construcțiilor prin inovare, sustenabilitate și relații de parteneriat solide.</p>
              </div>
            </div>
          </div>
        </section>
                ...
                {certificate.map((c, idx) => {
                  ...
                  const CardContent = (
                    <>
                      ...
                      {c.descriere && <p className="text-xs text-slate-500 leading-relaxed">{c.descriere}</p>}
                    </>
                  );
                  return hasFile ? (
        ...
        <section className="section-padding bg-brand-blue text-white">
          <div className="container-site text-center">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              <Link href="/contact" className="inline-flex items-center gap-2 bg-brand-accent text-white font-semibold px-8 py-3.5 rounded-xl hover:bg-brand-accent/90 transition-colors">Solicitați ofertă<svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg></Link>
